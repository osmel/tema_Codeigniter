...
class tree
{
	...
	public function get_node($id, $options = array()) {
		...
		if(isset($options['with_children'])) {
			$node['children'] = $this->get_children($id, isset($options['profundidad_hijo']));
		}
	...
	public function mv($id, $parent, $position = 0) {
		...
		$id			= $this->get_node($id, array('with_children'=> true, 'profundidad_hijo' => true, 'with_path' => true));
		if(!$parent['children']) {
			$position = 0;
		}
	...
	public function rm($id) {
		...
		$data = $this->get_node($id, array('with_children' => true, 'profundidad_hijo' => true));
