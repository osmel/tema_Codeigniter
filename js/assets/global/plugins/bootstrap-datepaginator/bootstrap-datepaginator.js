...
;(function($, window, document, undefined) {
	...
	DatePaginator.prototype = {
		...
		_unsubscribeEvents: function() {
			...
			this.$element.off('datosEnVivo'); //osmel
		...
		_subscribeEvents: function() {
			...
			if (typeof (this.options.ondatosEnVivo) === 'function') { //osmel
				this.$element.on('datosEnVivo', this.options.ondatosEnVivo);
			}
		...
		_setSelectedDate: function(selectedDate) {
			...
			if ((!selectedDate.isSame(this.options.selectedDate)) &&
				...
				//this.$element.trigger('datosEnVivo', [selectedDate.clone()]); //osmel
		...
		_buildData: function() {
			...
			for (var m = start; m.isBefore(end); m.add('days', 1)) {
				...
				data.items[data.items.length] = {
					...
					text: (m.isSame(this.options.selectedDate)) ? m.format(this.options.textSelected) : m.format(this.options.text), //aqui
